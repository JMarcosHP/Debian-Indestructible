# Debian Indestructible: Standard EFI System + Hibernation with Swap Partition

> *Traditional reliability meets modern snapshots—the time-tested approach.*
...
This guide walks you through installing Debian with BTRFS snapshots, using a dedicated swap partition for memory management and hibernation support. This configuration is ideal if you:

- Prefer the traditional, well-documented swap partition approach
- Want clear separation between swap and system data
- Need reliable hibernation capabilities (suspend-to-disk)
- Value simplicity and proven stability
...
- **Disk Space**: Minimum 70 GB for system + swap partition
...
**Notes:**
- Replace `/dev/device1`, `/dev/device2`, and `/dev/device3` with your actual device paths (e.g., `/dev/sda1`, `/dev/sda2`, `/dev/sda3`)
- Swap partition size: For hibernation, use `Total RAM + 1 GiB extra` (e.g., 17 GiB for 16 GiB RAM)
...
3. Create the **Swap Partition**:
   - Size: `Total RAM + 1GiB` (e.g., 17GiB for 16GiB RAM)
   - Type: `Linux swap`
...
Take note of the partition numbers created (e.g., `/dev/sda1`, `/dev/sda2`, `/dev/sda3`).

#### 7. Format Partitions

```bash
mkfs.vfat -F32 -n ESP /dev/device1
mkswap -L SWAP /dev/device2
mkfs.btrfs -L SYSTEM /dev/device3
```

Replace `/dev/device1`, `/dev/device2`, and `/dev/device3` with your actual partition paths.
...
SWAP="$(blkid -L SWAP)"
SWAP_UUID="$(blkid -t LABEL=SWAP -o value -s UUID)"
```
...
#### 22. Enable Swap Partition

```bash
swapon $SWAP
```

#### 23. Disable COW for Variable Data
...
#### 24. Bootstrap Debian Base System
...
#### 25. Bind-Mount System Directories
...
sed -i "s/dummy-esp/$ESP_UUID/g; s/dummy-swap/$SWAP_UUID/g; s/dummy-system/$SYSTEM_UUID/g" /mnt/etc/fstab
```
...
- Swap partition entry is present
...
#### 42. Define User Variable for Snapper
...
#### 43. Install GRUB Packages

```bash
apt install -y grub-efi-amd64 efibootmgr grub-efi-amd64-signed
```

#### 44. Configure Hibernation

Add the Swap UUID to enable hibernation:

```bash
cat > /etc/initramfs-tools/conf.d/resume << EOF
RESUME=UUID=$(blkid -t LABEL=SWAP -o value -s UUID)
EOF
```

#### 45. Verify Resume Configuration
...
#### 49. Clone and Configure grub-btrfs
...
#### 51. Unmount Snapshots Directory
...
#### 52. Create Snapper Configuration

```bash
snapper --no-dbus -c root create-config /
```

#### 53. Remount Snapshots Subvolume
...
#### 57. Install GRUB Rollback Plugin
...
#### 58. Unmount Everything and Reboot
...
Check that your swap partition is active:

```bash
swapon --show
```

Should display your swap partition with its size.

```bash
free -h
```

Should show swap space matching your partition size.
...
Congratulations! You've successfully built a Debian system with the traditional swap partition approach combined with modern snapshot capabilities. Your system can now:
...
- Hibernate and resume reliably with a dedicated swap partition
- Recover from almost any software catastrophe

**Advantages of Swap Partition:**
- Proven, well-documented approach used for decades
- Clear separation between system and swap space
- Simpler to manage and troubleshoot
- No filesystem overhead for swap management
